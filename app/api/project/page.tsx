...
const Project: React.FC = () => {
  ...
  {proj.map((item, index) => {
    ...
    const isApplied = item.appliedProjects.some(project => project.projectId === item.id);
  ...
  <Card className="w-full h-full flex flex-col justify-between">
    ...
    <CardFooter className="flex justify-center items-center">
      <button onClick={() => {handleAppply(item.alumni.id, item.id)}} className="bg-[#5169F6] px-5 py-2 rounded-lg text-white">Apply</button>
    </CardFooter>
  </Card>
</div>
